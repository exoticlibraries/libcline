...
CESTER_BODY(
    ...
    SETUP_XSLIST_FOR(User_ptr)
)

CESTER_TEST(print_str, _, {
    xslist(str) *fruits;

    cester_assert_uint_eq(xslist_new(str)(&fruits), XTD_OK);
    ...
    cester_assert_uint_eq(xslist_add(str)(fruits, "Pineapple"), XTD_OK);

	CESTER_CAPTURE_STDOUT();
	CLINE_XPRINT_XTD_CONTAINER(xslist, stdout, str, fruits, SUFFIX_6251425454);
	cester_assert_stdout_stream_content_equal("[\"Apple\", \"Banana\", \"Mango\", \"Pineapple\"]");

	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(str)(fruits, 1, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XSLIST(stdout, str, fruits);
	cester_assert_stdout_stream_content_equal("[\"Apple\", \"Mango\", \"Pineapple\"]");

	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(str)(fruits, 0, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xslist_remove_at(str)(fruits, 0, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xslist_remove_at(str)(fruits, 0, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XTD_CONTAINER(xslist, stdout, str, fruits, SUFFIX_656890909);
...
CESTER_TEST(print_int, _, {
    ...
    cester_assert_uint_eq(xslist_new(int)(&numbers), XTD_OK);
    cester_assert_uint_eq(xslist_add(int)(numbers, 10), XTD_OK);
    cester_assert_uint_eq(xslist_add(int)(numbers, 20), XTD_OK);
    cester_assert_uint_eq(xslist_add(int)(numbers, 30), XTD_OK);
    cester_assert_uint_eq(xslist_add(int)(numbers, 40), XTD_OK);

	CESTER_CAPTURE_STDOUT();
	CLINE_XPRINT_XTD_CONTAINER(xslist, stdout, int, numbers, SUFFIX_6251425454);
	cester_assert_stdout_stream_content_equal("[10, 20, 30, 40]");

	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(int)(numbers, 2, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XSLIST(stdout, int, numbers);
	cester_assert_stdout_stream_content_equal("[10, 20, 40]");

	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(int)(numbers, 0, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xslist_remove_at(int)(numbers, 0, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xslist_remove_at(int)(numbers, 0, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XTD_CONTAINER(xslist, stdout, int, numbers, SUFFIX_656890909);
	cester_assert_stdout_stream_content_equal("[]");
...
CESTER_TEST(print_struct, _, {
    ...
    cester_assert_uint_eq(xslist_new(User_ptr)(&users), XTD_OK);
    cester_assert_uint_eq(xslist_add(User_ptr)(users, &user1), XTD_OK);
    cester_assert_uint_eq(xslist_add(User_ptr)(users, &user2), XTD_OK);
    cester_assert_uint_eq(xslist_add(User_ptr)(users, &user3), XTD_OK);
	
	CESTER_CAPTURE_STDOUT();
	CLINE_XPRINT_XTD_CONTAINER_CUSTOM(xslist, stdout, User_ptr, users, user_to_string, SUFFIX_6251425454);
	...
	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(User_ptr)(users, 1, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XSLIST_CUSTOM(stdout, User_ptr, users, user_to_string);
	cester_assert_stdout_stream_content_equal("[{Name=thecarisma, Email=test@user.com}, {Name=vazrak, Email=vazrak@user.com}]");

	CESTER_RESET_STDOUT();
    cester_assert_uint_eq(xslist_remove_at(User_ptr)(users, 0, XTD_NULL), XTD_OK);
    cester_assert_uint_eq(xslist_remove_at(User_ptr)(users, 0, XTD_NULL), XTD_OK);
	CLINE_XPRINT_XTD_CONTAINER_CUSTOM(xslist, stdout, User_ptr, users, user_to_string, SUFFIX_656890909);
	cester_assert_stdout_stream_content_equal("[]");
...
CESTER_TEST(print_pointer, _, {
	...
	CESTER_CAPTURE_STDOUT();
	CLINE_XPRINT_XTD_CONTAINER(xslist, stdout, User_ptr, users, SUFFIX_6251425454);
